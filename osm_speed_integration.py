...
def snap_coords_list(
    coords: List[Tuple[float, float]], precision: int = 6
...
class OverpassSpeedFetcher:
    ...
    def fetch_speeds_for_bbox(
        ...
        try:
            print(
                f"  → Querying Overpass API (bbox: {max_lat-min_lat:.3f}° x {max_lon-min_lon:.3f}°)..."
            )
    ...
    def _parse_maxspeed(self, maxspeed_str: str) -> Optional[float]:
        ...
        maxspeed_str = (
            maxspeed_str.replace("km/h", "").replace("kmh", "").replace("kph", "").strip()
        )
...
def enrich_segments_with_osm_speeds(
    segments: List[dict],
    ...
    stats = {"kml": 0, "osm_matched": 0, "osm_fallback": 0, "default": 0}

    # Count and mark KML segments
    kml_count = sum(1 for seg in segments if seg.get("speed_limit") and seg["speed_limit"] > 0)
    ...
    print(
        f"  ✓ Speed sources: KML={stats['kml']}, OSM={stats['osm_matched']}, "
...
def build_graph_with_time_weights(
    segments: List[dict], treat_unspecified_as_two_way: bool = True, default_speed_kmh: float = 30.0
    ...
    if edges_without_speed > 0:
        ...
        print(
            f"  ⚠ {edges_without_speed}/{len(segments)} segments ({pct:.1f}%) "
...
def calculate_average_speed(segments: List[dict]) -> float:
    ...
    if total_distance > 0:
        ...
        print(
            f"  ✓ Calculated average speed: {avg_speed:.1f} km/h "
