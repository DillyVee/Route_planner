...
from PyQt6.QtWidgets import (
    ...
    QGroupBox, QRadioButton, QSpinBox, QDoubleSpinBox, QCheckBox,
    QProgressBar, QTabWidget, QMessageBox, QFrame, QScrollArea
)
...
def build_graph(segments, treat_unspecified_as_two_way=True):
    ...
    graph, required_edges = build_graph_with_time_weights(
        segments,
        treat_unspecified_as_two_way=treat_unspecified_as_two_way,
...
def greedy_arc_route_with_hungarian(graph, required_edges, seg_idxs, start_node=None, 
...
def centroid_of_cluster(cluster_idx_list, segments):
    ...
    lat = sum((segments[i]['start'][0]+segments[i]['end'][0])/2.0 for i in cluster_idx_list) / len(cluster_idx_list)
    lon = sum((segments[i]['start'][1]+segments[i]['end'][1])/2.0 for i in cluster_idx_list) / len(cluster_idx_list)
    return (lat, lon)
...
def write_mobile_gpx(full_coords, leg_breaks, leg_names, leg_distances, filename='mobile_route.gpx', avg_speed_kmh=30.0):
    ...
    desc.appendChild(doc.createTextNode(f'Optimized survey route: {len(leg_breaks)} legs, {total_dist/1000:.1f} km total'))
    ...
    for i, start_idx in enumerate(leg_breaks):
        ...
        wpt_name.appendChild(doc.createTextNode(leg_names[i] if i < len(leg_names) else f'Leg {i+1}'))
        ...
        wpt_desc.appendChild(doc.createTextNode(
...
def write_html_preview(coords, clusters, cluster_order, segments, filename='route_preview.html'):
    ...
    geo_collection = {
        ...
        "features": [route_geo] + cluster_features
    }
...
def full_pipeline(kml_path, cluster_method='auto', gx=10, gy=10, k_clusters=40,
    ...
    order = order_clusters(clusters, segments, use_ortools=use_ortools_for_cluster_order and ORTOOLS_AVAILABLE)
    ...
    gpx_file = write_mobile_gpx(
        full_route, leg_breaks, leg_names, leg_distances,
        filename=output_gpx, avg_speed_kmh=avg_speed_kmh
    )
    print(f"  ✓ GPX: {gpx_file}")
    
    html_file = write_html_preview(
        full_route, clusters, improved_order, segments,
    ...
    print(f"✓ Estimated travel time: {total_time_hours:.2f} hours ({total_time_hours*60:.0f} minutes)")
...
class RouteOptimizer(QMainWindow):
    ...
    def init_ui(self):
        ...
        title.setStyleSheet("color: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #00e5ff, stop:1 #8a2be2);")
        header_layout.addWidget(title)
        
        subtitle = QLabel('INTELLIGENT VEHICLE SURVEY ROUTE OPTIMIZATION')
        subtitle.setAlignment(Qt.AlignmentFlag.AlignCenter)
        subtitle.setStyleSheet("color: #a0a0aa; font-size: 11pt; letter-spacing: 1px; font-weight: 600;")
        ...
        log_label.setStyleSheet("font-weight: 700; color: #00e5ff; font-size: 11pt; margin-top: 8px;")
